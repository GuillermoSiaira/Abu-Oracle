"""Bridge script: raw or v1 dataset → HF Core v2 features (house + angular-aware).

Usage (raw JSONL):
        python scripts/generate_hf_dataset_v2.py \
            --input data/raw/raw_birthdata.jsonl \
            --output data/processed/hf_dataset_v2.parquet \
            --failures data/processed/hf_dataset_v2_failures.jsonl \
            --summary data/processed/hf_dataset_v2_summary.json

Usage (from existing hf_dataset_v1.parquet):
        python scripts/generate_hf_dataset_v2.py \
            --input data/processed/hf_dataset_v1.parquet \
            --output data/processed/hf_dataset_v2.parquet \
            --failures data/processed/hf_dataset_v2_failures.jsonl \
            --summary data/processed/hf_dataset_v2_summary.json
"""
...
for path in (REPO_ROOT, ABU_ROOT):
...
from abu_engine.harmony.schema_v2 import HOUSE_COUNT_COLS, HOUSE_WEIGHTED_COLS, HF_V2_METRICS
...
LOGGER = logging.getLogger("generate_hf_dataset_v2")

PIPELINE_VERSION = "hf_dataset_v2"
HF_CORE_VERSION = "HF Core v2"
...
# -------------------- Parsing helpers (reused from v1 script) --------------------
...
def angles_from_chart(lat: float, lon: float, dt_utc: datetime) -> Tuple[Dict[str, float], Dict[str, float], List[float]]:
    ...
    cusps = [float(c) % 360.0 for c in houses_data.get("cusps", [])]
    ...
    return angles, planet_positions, cusps
...
def field_features_v1(angles_deg: Dict[str, float]) -> Dict[str, float]:
...
def field_features_v2(angles_deg: Dict[str, float], cusps: List[float]) -> Dict[str, float]:
    field = aggregate_field_v2(angles_deg, cusps)
    out = {
        "hf_total_v2": float(field["HF_total_v2"]),
        "hf_harmony_v2": float(field["HF_harmony_v2"]),
        "hf_tension_v2": float(field["HF_tension_v2"]),
        "hf_conjunction_v2": float(field["HF_conjunction_v2"]),
        "hf_angularity": float(field["HF_angularity"]),
        "hf_house_balance": float(field["HF_house_balance"]),
    }

    # House occupancy columns
    house_feats = field.get("house_features", {})
    for col in HOUSE_COUNT_COLS + HOUSE_WEIGHTED_COLS:
        out[col] = float(house_feats.get(col, 0.0))
    out["house_weight_total"] = float(house_feats.get("house_weight_total", 0.0))
    out["house_entropy"] = float(house_feats.get("house_entropy", 0.0))
...
def deterministic_column_order() -> List[str]:
    ...
    hf_v1_cols = ["hf_total", "hf_harmony", "hf_tension", "hf_conjunction"]
    hf_v2_cols = HF_V2_METRICS + HOUSE_COUNT_COLS + HOUSE_WEIGHTED_COLS + ["house_weight_total", "house_entropy"]
    meta_cols = ["pipeline_version", "hf_core_version", "processed_at"]
    return base_cols + lon_cols + cv_cols + hk_cols + hf_v1_cols + hf_v2_cols + meta_cols
...
def load_input_df(input_path: Path) -> pd.DataFrame:
    """Load input as JSONL (raw) or Parquet (v1)."""
...
def process_row(row: pd.Series, processed_at_iso: str) -> Tuple[Optional[Dict[str, object]], Optional[Dict[str, object]]]:
    ...
    try:
        ...
        angles, planet_positions, cusps = angles_from_chart(float(row["latitude"]), float(row["longitude"]), utc_dt)
        ...
        record.update(field_features_v1(angle_list))
        record.update(field_features_v2(angle_list, cusps))
...
def compute_summary(
    ...
    return {
        ...
        "hf_total_v2": stats(successes.get("hf_total_v2", pd.Series(dtype=float))),
        "hf_harmony_v2": stats(successes.get("hf_harmony_v2", pd.Series(dtype=float))),
        "hf_tension_v2": stats(successes.get("hf_tension_v2", pd.Series(dtype=float))),
        "hf_conjunction_v2": stats(successes.get("hf_conjunction_v2", pd.Series(dtype=float))),
        "hf_angularity": stats(successes.get("hf_angularity", pd.Series(dtype=float))),
        "hf_house_balance": stats(successes.get("hf_house_balance", pd.Series(dtype=float))),
    }


def parse_args() -> Paths:
    parser = argparse.ArgumentParser(description="Generate HF Core v2 dataset from raw natal data")
...
def main() -> None:
    ...
    LOGGER.info("Loading dataset: %s", paths.input_path)
    df = load_input_df(paths.input_path)
    total_input = len(df)
    ...
    for _, row in tqdm(df.iterrows(), total=total_input, desc="Processing v2"):
