"""Generate HF Core v2 relocation grids (house- and angular-aware).

Mirrors `generate_relocation_field.py` but uses HF v2 scoring.
"""
...
EXPERIMENT_VERSION = "HF_RELOC_v2"
GRID_STEP_DEG = 5.0
HOUSE_SYSTEM = "Placidus"

DATASET_PATH = REPO_ROOT / "data" / "processed" / "hf_dataset_v2.parquet"
OUTPUT_DIR = REPO_ROOT / "output" / "relocation_fields_v2"
...
def compute_hf_metrics_v2(angles_deg: Dict[str, float], cusps: List[float]) -> Dict[str, float]:
    hf = aggregate_field_v2(angles_deg, cusps)
    return {
        "hf_total_v2": float(hf["HF_total_v2"]),
        "hf_harmony_v2": float(hf["HF_harmony_v2"]),
        "hf_tension_v2": float(hf["HF_tension_v2"]),
        "hf_conjunction_v2": float(hf["HF_conjunction_v2"]),
        "hf_angularity": float(hf["HF_angularity"]),
        "hf_house_balance": float(hf["HF_house_balance"]),
    }
...
def compute_natal_hf(subject: pd.Series, planet_positions: Dict[str, float]) -> Dict[str, float]:
    ...
    metrics = compute_hf_metrics_v2(angles, cusps)
    return {
        "angles": angles,
        "asc": angles["ASC"],
        "mc": angles["MC"],
...
def process_subject(subject: pd.Series, grid: List[Tuple[float, float]]) -> pd.DataFrame:
    ...
    for idx, (rel_lat, rel_lon) in enumerate(grid, start=1):
        ...
        row = {
            ...
            "hf_total_v2": float(metrics.get("hf_total_v2", float("nan"))),
            "hf_harmony_v2": float(metrics.get("hf_harmony_v2", float("nan"))),
            "hf_tension_v2": float(metrics.get("hf_tension_v2", float("nan"))),
            "hf_conjunction_v2": float(metrics.get("hf_conjunction_v2", float("nan"))),
            "hf_angularity": float(metrics.get("hf_angularity", float("nan"))),
            "hf_house_balance": float(metrics.get("hf_house_balance", float("nan"))),
            "delta_hf_total_v2": float(metrics.get("hf_total_v2", float("nan"))) - float(natal["hf_total_v2"])
            if valid
            else float("nan"),
            "delta_hf_harmony_v2": float(metrics.get("hf_harmony_v2", float("nan"))) - float(natal["hf_harmony_v2"])
            if valid
            else float("nan"),
            "delta_hf_tension_v2": float(metrics.get("hf_tension_v2", float("nan"))) - float(natal["hf_tension_v2"])
            if valid
            else float("nan"),
            "delta_hf_conjunction_v2": float(metrics.get("hf_conjunction_v2", float("nan"))) - float(natal["hf_conjunction_v2"])
            if valid
            else float("nan"),
