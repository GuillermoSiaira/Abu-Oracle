...
**Fecha:** 2026-04-01  
**Dataset:** `data/biographical_events/events_detailed.csv` (527 eventos, 26 sujetos)  
**Artefactos:** `abu-oracle-research/` (nuevo repositorio público)

### Objetivo
Consolidar y publicar los resultados de validación empírica del HF en formato reproducible.
Crear corpus público anonimizado + figuras + tabla de correlaciones por dominio.
...
df = pd.read_csv('data/biographical_events/events_detailed.csv')

# HF global (Pearson r)
pearson_r, p = stats.pearsonr(df['valence_num'], df['hf_weighted'])
# => r = 0.121, p = 0.005

# Cohen's d (positivo vs negativo, excluyendo neutral)
pos = df[df['valence'] == 'positive']['hf_weighted']
neg = df[df['valence'] == 'negative']['hf_weighted']
pooled_std = np.sqrt((pos.std()**2 + neg.std()**2) / 2)
cohens_d = (pos.mean() - neg.mean()) / pooled_std
# => d = 0.441

# Domain-filtered: resultados de correlate_by_domain.py (analysis/domain_correlation_results.json)
# Ejecutado previamente con abu_engine house_significators() + planet_subset per casa
```

### Resultados consolidados

**HF_v3 Global (baseline):**
- Pearson r = 0.121 (n=527), r = 0.133 (n=446, no-neutral)
- Cohen's d = 0.441 (positivo vs negativo)
- Media HF positivo = −12.010 · Media HF negativo = −13.098

**HF_v6 Domain-Filtered (segmentado por casa):**

| Casa | Dominio | N | r_global | r_domain | d_global | d_domain | Δr |
|------|---------|---|----------|----------|----------|----------|----|
| 5 | Creatividad | 57 | 0.200 | 0.350 | — | — | **+0.150** |
| 6 | Trabajo/Salud | 10 | −0.123 | −0.117 | — | — | +0.006 |
| 7 | Relaciones | 93 | 0.063 | 0.041 | 0.207 | 0.055 | −0.022 |
| 9 | Expansión | 66 | −0.063 | −0.046 | −0.221 | −0.175 | +0.017 |
| 10 | Carrera | 250 | 0.074 | 0.013 | 0.567 | 0.056 | −0.061 |
| 12 | Inconsciente | 12 | −0.070 | −0.377 | — | — | −0.307 |

### Diagnósticos críticos

**H05 Creatividad** es el resultado más reproducible: el filtrado de dominio mejora la correlación en +0.150. Señal emergente con corpus de 57 eventos.

**H10 Carrera** tiene el mayor d_global (0.567) pero el filtro colapsa la señal (0.056). Causa: significadores de H10 incluyen planetas lentos (Neptuno, Plutón) con baja varianza temporal. El dataset de H10 tiene 231 positivos vs 4 negativos — desbalance severo que impide separación estadística con el subconjunto planetario de dominio.

**Cohen's d NaN** en múltiples casas = N+ < 2 o N- < 2 en ese dominio. No es falla del modelo — es escasez de corpus de eventos negativos en esos dominios.

### Comando reproducible
```bash
cd d:/projects/ai-oracle
python scripts/correlate_by_domain.py
# Output: analysis/domain_correlation_results.json + analysis/domain_correlation_report.md
```

### Artefactos
- `abu-oracle-research/data/corpus/events_public.csv` — corpus anonimizado público
- `abu-oracle-research/data/corpus/events_summary.csv` — estadísticas agregadas
- `abu-oracle-research/data/results/correlation_v3_global.json` — validación global
- `abu-oracle-research/data/results/domain_correlation_table.csv` — validación por dominio
- `abu-oracle-research/figures/corpus_overview.png` — panel 2×2 corpus (Instagram)
- `abu-oracle-research/figures/hf_valence_distribution.png` — distribución HF por valencia (Twitter)
- `abu-oracle-research/figures/domain_correlation_heatmap.png` — heatmap por dominio
- `abu-oracle-research/figures/cohens_d_by_domain.png` — Cohen's d por dominio
- `abu-oracle-research/hashes/SHA256SUMS.txt` — integridad de archivos
